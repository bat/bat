...
\Icode{BCModel} contains a \Icode{BCParameterSet} member, which can be
accessed directly through
\begin{verbatim}
BCParameterSet * BCModel::GetParameterSet()
void * BCModel::SetParameterSet(BCParameterSet * parset),
\end{verbatim}
where \Icode{BCParameterSet} is a \Icode{typedef} of a vector of
pointers to \Icode{BCParameter} objects.

One can also access the parameter set indirectly to add parameters
through
\begin{verbatim}
...
\begin{verbatim}
BCModel::LogLikelihood(std::vector<double> parameters),
\end{verbatim}
...
\end{verbatim}
where \Icode{BCMarginizationMethod} is \Icode{enum} defined in
\Icode{BCIntegrate} with the values \Icode{kMargMonteCarlo} for
uncorrelated Monte Carlo sampling, and \Icode{kMargMetropolis}
(default) for correlated sampling using a Markov Chain Monte Carlo.
