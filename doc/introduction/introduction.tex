...
install \bat. If you are installing \bat\ e.g. in your \verb|$HOMEDIR|, %$ workaround for bad highlighting
you need to add the path to the library and to the
...
If the parameters are considered independent in the prior probability, then

\noindent
\verb|double BCModel::LogAPrioriProbability(std::vector <double> parameters)|
need not be overloaded.
Alternatively, they can be set for individual parameters through \verb|TF1| 
function objects using
\begin{verbatim}
...
\end{verbatim}
The prior maybe chosen constant over one or all
dimensions of the parameter space using
\begin{verbatim}
int SetPriorConstant(int index);
int SetPriorConstantAll();
\end{verbatim}
Note that the correct constant is calculated from the parameter ranges only 
once, and then cached for reuse during the Markov chain run. For simple
problems,  caching can significantly speed up the execution, while for
complicated likelihoods, the time spent calculating the prior probability
is usually negligible.

In case the prior is undefined for one of the parameters, a constant prior
for that parameter is assumed, and a warning is issued.
% --------------------------------------------------------
...
page. Three working examples are provided with the current release.
...
scaled to the valid parameter range. \\
