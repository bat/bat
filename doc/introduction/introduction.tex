...
Poisson distributions. Several channels can be fitted
simultaneously. 
...
The main MTF class inherits from the \verb|BCModel| class and is named
\verb|BCMTF|. The constructor does not take any arguments, so a new
...
Since the MTF inherits on \verb|BCModel|, the fit can be started like
other fits in BAT, e.g., using one of the following methods:
%
\begin{verbatim}
...
\item \verb|PrintAllMarginalized(const char* name)| prints the
  ...
  the processes and nuisance parameters into the file \verb|name|.
\item \verb|PrintResults(const char* name)| writes a summary of the
  fit into the file \verb|name|.
\item 
\begin{verbatim} 
...
std::vector<TH1D> BCMTFAnalysisFacility::
                  BuildEnsemble(std::vector<double> parameters) , 
\end{verbatim}
%
...
\begin{verbatim}
std::vector<TH1D> BCMTFAnalysisFacility::
                  BuildEnsembles(std::vector<double> parameters, 
                                 int nensembles) , 
\end{verbatim}
%
...
TTree* BCMTFAnalysisFacility::
       PerformEnsembleTest(std::vector<double> parameters,
                           int nensembles) ,
\end{verbatim}
%
...
The template fitter is formulated in terms of Bayesian reasoning. The
...
\subsubsection{Including systematic uncertainties}
