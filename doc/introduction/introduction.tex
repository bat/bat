...
\end{verbatim}
When MCMC marginalization is run, the parameter's current ranges are used.
% --------------------------------------------------------
...
%
\begin{itemize}
\item \verb|BCModel::kIntMonteCarlo|. Sample-mean integration (default if Cuba is not available).
\item \verb|BCModel::kIntImportance|. Importance sampling. Note that
  there is no good default choice for the proposal function, so the
  user has to overload\\
  \verb|BCIntegrate::EvalSampling(const std::vector<double> & x)|
