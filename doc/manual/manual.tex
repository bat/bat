...
\emph{proposal distribution} $q(y \cond x, \xi)$ with adjustable parameters
$\xi$. The main difference between MCMC algorithms is typically given
by different choices of $q$. The Metropolis algorithm doesn't specify
which $q$ to choose, so we can and have to select a function $q$ and
tune $\xi$ according to our needs.
...
point $x$ and not on previous points. If the value of $\xi$ is set
based on a past sequence of iterations of the chain, we need two
stages of sampling in \BAT, the \emph{prerun} and the \emph{main
  run}. In the prerun, the chain is run and periodically $\xi$ is
updated based on the past iterations. In contrast, $\xi$ is kept fixed
in the main run to have a proper Markov chain. \fred{A flow diagram
  might help}

\BAT offers two kinds of proposal function termed \emph{factorized} and \emph{multivariate}.
...
one iteration of the chain once a new point has been proposed in \emph{every}
direction. This means the chain attempts to perform a sequence of
axis-aligned moves in one iteration.

\fred{Easiest to understand would be pseudocode} SetN
...
\subsection{Prerun}
\fred{Explain main options to adjust length of prerun, updates etc.,
  discuss proposal-specific options below} The prerun terminates after
a maximum number of iterations or if multiple chains mix according to
the R value. Optionally the minimum/maximum number of iterations can
be set with \code{SetNIterationsPreRun[Min|Max]}.
