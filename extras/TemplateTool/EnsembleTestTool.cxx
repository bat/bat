...
// ---------------------------------------------------------
EnsembleTestTool::EnsembleTestTool() : fTemplateModel(0)
															 , fFile(0)
															 , fTree(0)
															 , fEnsembleCounter(0)
...
int EnsembleTestTool::PerformEnsembleTest()
{
	...
	for(int j = 0; j < fNEnsembles; j++){
      ...
      cout << "Fraction of ensembles analyzed: " << double(j+1) / double(fNEnsembles) * 100 << "%" << std::endl;
		...
		fTemplateModel->SetData(*ensemble);

		// find mode
		fTemplateModel->FindMode();

		// perform MCMC
		if(fFlagMCMC) {
			fTemplateModel->MarginalizeAll();

			// get number of parameters
			int npar = fTemplateModel->GetNParameters();
			
			// loop over parameters and set tree variables
			for (int i = 0; i < npar; ++i) {
				BCH1D* hist = fTemplateModel->GetMarginalized(fTemplateModel->GetParameter(i)); 
		...
		if (fFlagMCMC) {
			int nratios = fTemplateModel->GetNRatios(); 
			for (int i = 0; i < nratios; ++i) {				
				TH1D histtemp = fTemplateModel->GetHistRatio1D(i); 
		...
		// set tree variables
		fOutParModeGlobal      = fTemplateModel->GetBestFitParameters();
		fOutParErrorUpGlobal   = fTemplateModel->GetBestFitParameterErrors();
		fOutParErrorDownGlobal = fTemplateModel->GetBestFitParameterErrors();
		fOutChi2               = fTemplateModel->CalculateChi2(); 
		fOutNDF                = fTemplateModel->GetNDF();
		fOutChi2Prob           = fTemplateModel->CalculateChi2Prob(); 
		fOutKSProb             = fTemplateModel->CalculateKSProb(); 
		fOutPValue             = fTemplateModel->CalculatePValue(); 
		fOutNEvents            = int(fTemplateModel->GetData().Integral());
