...
// ---------------------------------------------------------
int BCIntegrate::PrintAllMarginalized1D(const char * filebase, std::string filetype )
{
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (GetMarginalized(a)){
      GetMarginalized(a)->Print(Form("%s_1D_%s.%s", filebase, a->GetName().data(),filetype.c_str()));
...
// ---------------------------------------------------------
int BCIntegrate::PrintAllMarginalized2D(const char * filebase, std::string filetype)
{
  if (fMCMCH2Marginalized.size() == 0) {
    BCLog::OutError("BCIntegrate::PrintAllMarginalized : Marginalized distributions not available.");
    return 0;
  }
  if(filetype=="C" or filetype==".C" or filetype==".c" or filetype=="c"){
    BCLog::OutError("BCIntegrate::PrintAllMarginalized2D : filetype .C is not supported, printing .root files instead");
    filetype="root";
  }
  int k = 0;
  int n = GetNParameters();
  for (int i = 0; i < n - 1; i++) {
	...
	GetMarginalized(a, b)->Print(Form("%s_2D_%s_%s.%s",filebase, a->GetName().data(), b->GetName().data(),filetype.c_str()));
...
int BCIntegrate::PrintAllMarginalized(const char * file, std::string options1d, std::string options2d, unsigned int hdiv, unsigned int vdiv)
{
  ...
  if((filename.substr(filename.find_last_of(".")+1) == "root")){
    useRootFile=true;
    outPutRootFile=new TFile(filename.c_str(),"recreate");
  }
  ...
  if ( (filename.find_last_of(".") == std::string::npos) or
	...
	(filename.substr(filename.find_last_of(".")+1) != "ps") and useRootFile==false)) {
    // make it a PDF file
    filename += ".pdf";
  }
  ...
  for (unsigned i = 0; i < fParameters.Size(); ++i) {
    ...
    if (n != 0 && n % (hdiv * vdiv) == 0) {
      ...
      if ( n <= (hdiv * vdiv)) {
	if(useRootFile){
	  outPutRootFile->WriteTObject(&c,TString::Format("c_Parameter_%i",i-1));
	}else{
  ...
  for (unsigned i = 0; i < fParameters.Size() - 1; ++i) {
	...
	if(useRootFile){
	  if(k==0){
	    outPutRootFile->WriteTObject(&c,TString::Format("c_Parameter_%i",fParameters.Size()-1));
	  }else{
	    if(j==i+1){
	      outPutRootFile->WriteTObject(&c,TString::Format("c_Parameter_%i_vs_%i",i-1,fParameters.Size()-1));
	    }else{
	      outPutRootFile->WriteTObject(&c,TString::Format("c_Parameter_%i_vs_%i",i,j-1));
	    }
	  }
	}else{
	  c.Print(filename.c_str());
  ...
  BCLog::OutSummary(Form("Printing all marginalized distributions done, %d pages used",nPages));
  if(useRootFile){
    outPutRootFile->WriteTObject(&c,TString::Format("c_Parameter_%i_vs_%i",fParameters.Size()-2,fParameters.Size()-1));
