...
BCParameter * BCModel::GetParameter(int index)
{
   ...
   if (index < 0 || index >= (int) this -> GetNParameters()) {
      BCLog::OutWarning(Form(
            "BCModel::GetParameter. Parameter index %d not within range.",
            index));
      return 0;
   }
...
void BCModel::SetNbins(const char * parname, int nbins)
{
   ...
   if (!p) {
      BCLog::OutWarning(Form(
            "BCModel::SetNbins : Parameter '%s' not found so Nbins not set",
            parname));
      return;
   }
...
TGraph * BCModel::GetErrorBandGraph(double level1, double level2)
{
   ...
   for (int i = 0; i < nx; i++) {
      graph -> SetPoint(i, fErrorBandXY -> GetXaxis() -> GetBinCenter(i + 1),
            ymin.at(i));
      graph -> SetPoint(nx + i, fErrorBandXY -> GetXaxis() -> GetBinCenter(nx
            - i), ymax.at(nx - i - 1));
   }
...
TH2D * BCModel::GetErrorBandXY_yellow(double level, int nsmooth)
{
   ...
   TH2D * hist_tempxy = (TH2D*) fErrorBandXY -> Clone(TString::Format(
         "%s_sub_%f.2", fErrorBandXY->GetName(), level));
   hist_tempxy -> Reset();
   ...
   for (int ix = 1; ix < nx; ix++) {
      ...
      TH1D * hist_temp_yellow =
            hist_temp -> GetSmallestIntervalHistogram(level);

      for (int iy = 1; iy <= ny; ++iy)
         hist_tempxy -> SetBinContent(ix, iy,
               hist_temp_yellow -> GetBinContent(iy));
...
bool BCModel::GetFlagBoundaries()
{
   ...
   if (fDataPointLowerBoundaries -> GetNValues()
         != fDataSet -> GetDataPoint(0) -> GetNValues())
      return false;

   if (fDataPointUpperBoundaries -> GetNValues()
         != fDataSet -> GetDataPoint(0) -> GetNValues())
      return false;
...
void BCModel::SetDataBoundaries(unsigned int index, double lowerboundary,
      double upperboundary, bool fixed)
{
   ...
   if (!fDataPointLowerBoundaries)
      fDataPointLowerBoundaries = new BCDataPoint(
            fDataSet -> GetDataPoint(0) -> GetNValues());

   if (!fDataPointUpperBoundaries)
      fDataPointUpperBoundaries = new BCDataPoint(
            fDataSet -> GetDataPoint(0) -> GetNValues());
...
void BCModel::SetErrorBandContinuous(bool flag)
{
   ...
   for (unsigned int i = 0; i < fDataSet -> GetNDataPoints(); ++i)
      fErrorBandX.push_back(fDataSet -> GetDataPoint(i) -> GetValue(
            fFitFunctionIndexX));
}
...
int BCModel::AddParameter(BCParameter * parameter)
{
   ...
   int flag_exists = 0;
   for (unsigned int i = 0; i < this -> GetNParameters(); i++)
      if (this -> CompareStrings(parameter -> GetName().data(),
            this -> GetParameter(i) -> GetName().data()) == 0)
         flag_exists = -1;
...
double BCModel::SamplingFunction(std::vector<double> parameters)
{
   ...
   for (std::vector<BCParameter*>::const_iterator it = fParameterSet -> begin(); it
         != fParameterSet -> end(); ++it)
      probability *= 1.0
            / ((*it) -> GetUpperLimit() - (*it) -> GetLowerLimit());
   return probability;
}
...
{
   BCLog::OutSummary(Form("Model \'%s\': Normalizing probability",
         this->GetName().data()));
...
void BCModel::FindMode(std::vector<double> start)
{
   ...
   BCLog::OutSummary(Form("Model \'%s\': Finding mode",
         this -> GetName().data()));
   ...
   BCLog::OutError(Form("BCModel::FindMode : Invalid mode finding method: %d",
...
void BCModel::WriteMode(const char * file)
{
   ...
   ofi << "#" << std::endl;
   ofi
         << "#  This file was created automatically by BCModel::WriteMode() call."
         << std::endl;
...
int BCModel::ReadMode(const char * file)
{
   ...
   if (i < npar) {
      BCLog::OutError(Form(
            "BCModel::ReadMode : Couldn't read mode from file %s.", file));
      BCLog::OutError(Form(
            "BCModel::ReadMode : Expected %d parameters, found %d.", npar, i));
      return 0;
   }

   BCLog::OutSummary(
         Form(
               "#  Read in best fit parameters (mode) for model \'%s\' from file %s:",
               fName.data(), file));
   this->SetMode(mode);
...
{
   BCLog::OutSummary(Form("Running MCMC for model \'%s\'",
         this->GetName().data()));
   ...
   if (fFitFunctionIndexX >= 0) {
      ...
      fErrorBandXY
            = new TH2D(TString::Format("errorbandxy_%d", BCLog::GetHIndex()),
                  "", fErrorBandNbinsX, fDataPointLowerBoundaries -> GetValue(
                        fFitFunctionIndexX) - .5 * dx,
                  fDataPointUpperBoundaries -> GetValue(fFitFunctionIndexX)
                        + .5 * dx, fErrorBandNbinsY,
                  fDataPointLowerBoundaries -> GetValue(fFitFunctionIndexY)
                        - .5 * dy, fDataPointUpperBoundaries -> GetValue(
                        fFitFunctionIndexY) + .5 * dy);
...
BCH1D * BCModel::GetMarginalized(BCParameter * parameter)
{
   ...
   // set axis labels
   hist->SetName(Form("hist_%s_%s", GetName().data(),
         parameter->GetName().data()));
...
int BCModel::ReadMarginalizedFromFile(const char * file)
{
   ...
   if (!froot->IsOpen()) {
      BCLog::OutError(Form(
            "BCModel::ReadMarginalizedFromFile : Couldn't open file %s.", file));
      return 0;
   }
   ...
   for (int i = 0; i < n; i++) {
      ...
      TKey * key = froot -> GetKey(TString::Format("hist_%s_%s",
            this -> GetName().data(), a -> GetName().data()));
      if (key) {
   ...
   for (int i = 0; i < n - 1; i++) {
      for (int j = i + 1; j < n; j++) {
         ...
         TKey * key = froot -> GetKey(TString::Format("hist_%s_%s_%s",
               this -> GetName().data(), a -> GetName().data(),
               b -> GetName().data()));
         if (key) {
...
int BCModel::ReadErrorBandFromFile(const char * file)
{
   ...
   if (!froot->IsOpen()) {
      BCLog::OutWarning(Form(
            "BCModel::ReadErrorBandFromFile. Couldn't open file %s.", file));
      return 0;
   }
...
int BCModel::PrintAllMarginalized1D(const char * filebase)
{
   ...
   for (int i = 0; i < n; i++) {
      ...
      if (this -> GetMarginalized(a))
         this -> GetMarginalized(a) -> Print(Form("%s_1D_%s.ps", filebase,
               a->GetName().data()));
   }
   ...
   for (int i = 0; i < n - 1; i++) {
      for (int j = i + 1; j < n; j++) {
         ...
         if (this -> GetMarginalized(a, b))
            this -> GetMarginalized(a, b) -> Print(Form("%s_2D_%s_%s.ps",
                  filebase, a->GetName().data(), b->GetName().data()));
         k++;
      }
   }
...
int BCModel::PrintAllMarginalized(const char * file, unsigned int hdiv,
      unsigned int vdiv)
{
   if (!fMCMCFlagFillHistograms) {
      BCLog::OutError(
            "BCModel::PrintAllMarginalized : Marginalized distributions not filled.");
      return 0;
   }
...
BCH2D * BCModel::GetMarginalized(BCParameter * par1, BCParameter * par2)
{
   ...
   if (fMCMCFlagsFillHistograms[index1] == false
         || fMCMCFlagsFillHistograms[index2] == false) {
      BCLog::OutError(
            Form(
                  "BCModel::GetMarginalized : Distribuion for '%s' and/or '%s' not filled.",
   ...
   if (index1 == index2) {
      BCLog::OutError(
            "BCModel::GetMarginalized : Provided parameters are identical. Distribution not available.");
      return 0;
   }
   ...
   // set axis labels
   hist->SetName(Form("hist_%s_%s_%s", GetName().data(),
         npar1->GetName().data(), npar2->GetName().data()));
   hist->SetXTitle(Form("%s", npar1->GetName().data()));
   hist->SetYTitle(Form("%s", npar2->GetName().data()));
   hist->SetStats(kFALSE);

   double gmode[] = { fBestFitParameters.at(index1), fBestFitParameters.at(
         index2) };
...
double BCModel::HessianMatrixElement(BCParameter * par1, BCParameter * par2,
...
void BCModel::PrintSummary()
{
   ...
   // model summary
   std::cout << std::endl << "   ---------------------------------"
         << std::endl << "    Model : " << fName.data() << std::endl
         << "   ---------------------------------" << std::endl
         << "     Index                : " << fIndex << std::endl
         << "     Number of parameters : " << nparameters << std::endl
         << std::endl << "     - Parameters : " << std::endl << std::endl;
   ...
   if (this -> GetBestFitParameters().size() > 0) {
      std::cout << std::endl << "     - Best fit parameters :" << std::endl
            << std::endl;

      for (int i = 0; i < nparameters; i++) {
         std::cout << "       " << fParameterSet -> at(i) -> GetName().data()
               << " = " << this -> GetBestFitParameter(i) << " (overall)"
               << std::endl;
   ...
   // normalization
   if (fNormalization > 0)
      std::cout << "     Normalization        : " << fNormalization
            << std::endl;
}
...
void BCModel::PrintResults(const char * file)
{
   ...
   if (fMCMCFlagRun && fMCMCFlagFillHistograms) {
      ...
      for (int i = 0; i < npar; ++i) {
         ...
         ofi << "  (" << i << ") Parameter \""
               ...
               << std::setprecision(4) << bch1d->GetQuantile(0.84)
               - bch1d->GetMedian() << " - " << std::setprecision(4)
         ...
         for (int j = 0; j < ninter; j += 5)
            ofi << "       " << v.at(j) << " - " << v.at(j + 1)
                  << " (local mode at " << v.at(j + 3) << " with rel. height "
                  << v.at(j + 2) << "; rel. area " << v.at(j + 4) << ")"
                  << std::endl;
   ...
   if (int(fBestFitParameters.size()) > 0) {
      ...
      for (int i = 0; i < npar; ++i)
         ofi << "  (" << i << ") Parameter \""
               << fParameterSet->at(i)->GetName().data() << "\": "
               << fBestFitParameters.at(i) << " +- "
               << fBestFitParameterErrors.at(i) << std::endl;
      ofi << std::endl;
   } else {
   ...
   if (fMCMCFlagRun) {
      ofi << " Status of the MCMC" << std::endl << " =================="
            << std::endl << " Convergence reached: " << (flag_conv ? "yes"
            : "no") << std::endl;
      ...
      if (flag_conv)
         ...
         ofi << " WARNING: the Markov Chain did not converge! Be\n"
               ...
               << std::endl;
      ofi << " Number of chains:                       " << MCMCGetNChains()
            << std::endl << " Number of iterations per chain:         "
            << MCMCGetNIterationsRun() << std::endl << " Average efficiencies:"
            << std::endl;
      ...
      for (int ipar = 0; ipar < npar; ++ipar)
         for (int ichain = 0; ichain < nchains; ++ichain) {
            ...
            efficiencies[ipar] += double(MCMCGetNTrialsTrue().at(index))
                  / double(MCMCGetNTrialsTrue().at(index)
                        + MCMCGetNTrialsFalse().at(index)) / double(nchains)
                  * 100.;
         }
...
void BCModel::PrintShortFitSummary(int chi2flag)
{
   ...
   BCLog::OutSummary(Form("   Number of parameters:  Npar  = %i",
         GetNParameters()));
   BCLog::OutSummary(Form("   Number of data points: Ndata = %i",
         GetNDataPoints()));
   BCLog::OutSummary("   Number of degrees of freedom:");
   BCLog::OutSummary(Form("      NDoF = Ndata - Npar = %i", GetNDataPoints()
         - GetNParameters()));

   BCLog::OutSummary("   Best fit parameters (global):");
   for (unsigned int i = 0; i < GetNParameters(); ++i)
      BCLog::OutSummary(Form("      %s = %.3g",
            GetParameter(i)->GetName().data(), GetBestFitParameter(i)));

   BCLog::OutSummary("   Goodness-of-fit test:");
   BCLog::OutSummary(Form("      p-value = %.3g", GetPValue()));
   if (chi2flag) {
      BCLog::OutSummary(Form("      p-value corrected for NDoF = %.3g",
            GetPValueNDoF()));
...
void BCModel::PrintHessianMatrix(std::vector<double> parameters)
{
   ...
   // print to screen
   std::cout << std::endl << " Hessian matrix elements: " << std::endl
         << " Point: ";
   ...
   for (unsigned int i = 0; i < GetNParameters(); i++)
      for (unsigned int j = 0; j < i; j++) {
         ...
         // print to screen
         std::cout << " " << i << " " << j << " : " << hessianmatrixelement
               << std::endl;
      }
}
